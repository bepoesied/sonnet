defmodule SonnetWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: SonnetWeb.Gettext
  ...
  the closing/canceling of the modal, for example:
      ...
      <.modal id="confirm" on_cancel={JS.patch(~p"/posts")}>
        Are you sure?
        <:confirm>OK</:confirm>
        <:cancel>Close</:cancel>
      </.modal>
        ...
        <div class="flex min-h-full items-center justify-center p-4">
          <div class="w-full max-w-3xl">
            <div
              ...
              phx-key="escape"
              class="relative bg-base-100 p-6 sm:p-14 shadow-2xl rounded-2xl ring-1 ring-base-content/10 transition"
            >
              ...
              <div id={"#{@id}-content"}>
                ...
                {render_slot(@inner_block)}
                <div :if={@confirm != [] or @cancel != []} class="mt-8 flex items-center gap-5">
                  <%= for confirm <- @confirm do %>
                    <.button id={"#{@id}-confirm"} phx-disable-with>
                      {render_slot(confirm)}
                    </.button>
                  <% end %>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(SonnetWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(SonnetWeb.Gettext, "errors", msg, opts)
    end
  end
